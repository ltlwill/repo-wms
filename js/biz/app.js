...
;(function($){
	...
	App.webviews = {
		...
		receiving: {id:'pages/receiving.html',url:'pages/receiving.html'},      // 应用收件功能页
		pickup: {id:'pages/pickup.html',url:'pages/pickup.html'},               // 应用取件功能页
		shelves: {id:'pages/shelves.html',url:'pages/shelves.html'},   			// 应用上架功能页
		query: {id:'pages/query.html',url:'pages/query.html'},   			    // 应用查询功能页
		queryList: {id:'queryList.html',url:'queryList.html'},   			    // 应用查询结果列表功能页
		queryStatistics: {id:'pages/queryStatistics.html',url:'pages/queryStatistics.html'},   // 应用上架统计查询结果列表功能页
		material: {id:'pages/material.html',url:'pages/material.html'},   	    // 应用建料功能页
		...
		more: {id:'pages/more.html',url:'pages/more.html'},			            // 应用更多功能页 
		factoryReceiving: {id:'pages/factory_receiving.html',url:'pages/factory_receiving.html'},// 应用工厂收货功能页
		pickingOff: {id:'pages/picking_off.html',url:'pages/picking_off.html'}, // 应用工厂拣货下架功能页
		pickingOffSub: {id:'pages/picking_off_sub.html',url:'pages/picking_off_sub.html'}, // 应用工厂拣货下架子功能页
	};
	// 应用业务请求地址管理
	App.server = {};
	App.server.host = 'http://**********'; // 正式环境
//	App.server.host = 'http://192.168.2.50';            // 测试环境
//	App.server.host='http://192.168.2.108:9881';
	App.server.contextPath = '';// 正式环境 
	App.server.address = App.server.host + App.server.contextPath + ''; 
	// 登录授权地址
	App.server.oauth = {};
	App.server.oauth.host = 'http://**********';
	App.server.oauth.address = App.server.oauth.host + '/app';
	// 应用常量定义
	App.Constants = { 
		...
		CURRENT_NETWORK_ROUTE: 'current_network_route', // 当前选择的网络路线
	}; 
	// 创建URL（自动拼接当前的域名和上下文） 
	App.createUrl = function(url){
		url = url ? (url.indexOf('/') == 0 ? url : '/' + url) : '';
//		return App.server.address + url;
		var address = App.getNetRouteInfo(),
			address = address ? address : App.server.address;
		return address + url;
	};
	// 创建授权URL（自动拼接当前的域名和上下文）
	App.createOAuthUrl = function(url){
		url = url ? (url.indexOf('/') == 0 ? url : '/' + url) : '';
		return App.server.oauth.address + url;
	};
	// 获取公共的请求参数（如token等信息）
	...
	// 获取设置的网络路线
	App.getNetRouteInfo = function(){
		return localStorage.getItem(App.Constants.CURRENT_NETWORK_ROUTE);
	};
	// 设置网络路线
	App.setNetRouteInfo = function(val){
		localStorage.setItem(App.Constants.CURRENT_NETWORK_ROUTE,val);
	};
	// 更新token信息
	App.refreshTokenInfo = function(){
		$.doGet(App.createOAuthUrl('/refreshTokenInfo'),{},function(data){
			data = data || {};
	...
	App.requestSysSettings = function(callback){
		mui.doGet(App.createOAuthUrl('/getSysSettings'),{},function(result){
			result = result || [];
	...
	App.checkOrUpdateApp = function(auto){
		mui.doGet(App.server.oauth.host + '/appspace/easy_office_app_update.json?_r' + Math.random(),{},function(result){
...
//			timeout: (typeof opts.timeout == 'undefined' ? 60000 : opts.timeout), // 60s
			timeout: (typeof opts.timeout == 'undefined' ? 0 : opts.timeout), // 0表示永不超时
