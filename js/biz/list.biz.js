...
//				mui.fire(me.getSubWebview(),'unSelAllImages');
			...
			window.addEventListener('updateSelText',function(evt){
				me.updateSelText(evt.detail.text);
			});
			// 设置工具栏的状态（显示/不显示）
		...
		closeSelectMode: function(){
			this.table.imgGrid('closeSelectMode');
		},
		updateSelText: function(text){
			this.selAllBtn[0].querySelector('.mui-tab-label').innerText = text;
		},
		// 设置工具栏样式
